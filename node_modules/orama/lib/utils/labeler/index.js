...
var Labeler = function Labeler() {
  ...
  var move_to_new_pos = function move_to_new_pos(delta_energy, currT, i) {
    //if false keep old position otherwise take new one
    return Math.random() < Math.exp(-delta_energy / currT);
  };

  var select_label_index = function select_label_index() {
    //chose a random label from the ones that are movable
    return movable[Math.floor(Math.random() * movable.length)];
  };

  var set_movable_labels = function set_movable_labels() {
    //determine which labels can move after a label selection change

    var unchanged_from_last_selection = function unchanged_from_last_selection(label_index) {
      var _id = lab[label_index]._id;
      return _id in past_state && past_state[_id].anchor.x === anc[label_index].x && past_state[_id].anchor.y === anc[label_index].y;
    };

    var not_in_current_label_set = function not_in_current_label_set(_id) {
      for (var i in lab) {
    ...
    var removed_labels = Object.keys(past_state).filter(not_in_current_label_set).map(function (_id) {
      return past_state[_id].label;
    });

    var linear_distance = function linear_distance(x1, y1, x2, y2) {
    ...
    var near_new_label = function near_new_label(label_index) {
      var label = lab[label_index];
      var threshold = 10 * label.name.length;
      for (var i = 0; i < rerun_near.length; i++) {
        if (linear_distance(label.x, label.y, rerun_near[i].x, rerun_near[i].y) <= threshold) return true;
      }
      return false;
    };
  ...
  var initialize_labels = function initialize_labels() {
